#!/usr/bin/python3
"""
This module contains tests for the load_from_file method of the Base class.
"""
import unittest
import os
...
def test_load_rectangle(self):
    ...
    for i in range(len(my_list)):
        self.assertEqual(my_list_loaded[i].to_dictionary(), my_list[i]
                         .to_dictionary())
    os.remove("Rectangle.json")
...
def test_load_square(self):
    ...
    self.assertEqual(len(my_list), len(my_list_loaded))
    for i in range(len(my_list)):
        self.assertEqual(my_list_loaded[i]
                         .to_dictionary(), my_list[i].to_dictionary())
    os.remove("Square.json")
